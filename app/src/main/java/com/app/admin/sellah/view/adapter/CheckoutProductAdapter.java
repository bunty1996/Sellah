...
public class CheckoutProductAdapter extends RecyclerView.Adapter<CheckoutProductAdapter.CheckOutItemHolder> {
    ...
    public void onBindViewHolder(CheckOutItemHolder holder, final int position) {
        ...
        Glide.with(context).load(!arrayList.get(position).get("image").isEmpty() ? arrayList.get(position).get("image") : "")
            ...
            public void onClick(View v) {
                ...
                BuyerMakeOfferDialog.create(context, price, qty, new BuyerMakeOfferDialog.OnmakeOfferClick() {
                    ...
                    public void onMakeOfferClick(String price, String quantity, BuyerMakeOfferDialog buyerMakeOfferDialog) {
                        makeOfferApi(pos, price, arrayList.get(pos).get("name"), quantity, buyerMakeOfferDialog);
    ...
    private void makeOfferApi(int pos, String price, String name, String quantity, BuyerMakeOfferDialog dialog) {
        ...
        dialog1.show();
//        Log.e("prdctchekout",pos+"//"+arrayList.get(pos).get("id") +"//"+arrayList.get(pos).get("user_id")+"//"+ price+"//"+ "P"+"//"+ name+"//"+ quantity);
//        Log.e("prdctchekout",pos+"//"+arrayList.get(pos).get("id") +"//"+arrayList.get(pos).get("user_id")+"//"+ price+"//"+ "P"+"//"+ name+"//"+ quantity);
        Call<MakeOfferModel> makeOfferCall = service.makeOfferApi(auth,token,HelperPreferences.get(context).getString(UID), arrayList.get(pos).get("id"), arrayList.get(pos).get("user_id"), price, "P", name, quantity);
