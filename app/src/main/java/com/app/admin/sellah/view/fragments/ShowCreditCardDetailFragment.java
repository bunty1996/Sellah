...
public class ShowCreditCardDetailFragment extends AppCompatActivity {
    ...
    private void stripeApiHit(String token1, Dialog dialog) {
      ...
      //  Log.e("showcreditcard", auth + "/" + token + "/" + HelperPreferences.get(ShowCreditCardDetailFragment.this).getString(UID) + "/" + productId + "/" + "50" + "/" + "usd" + "/" + token1 + "/" + customer_id + "/" + "Y");
        Call<JsonObject> stripePaymentApi = webService.stripePayment(auth, token, HelperPreferences.get(ShowCreditCardDetailFragment.this).getString(UID), productId, "50", "usd", token1, "", "", customer_id, "Y");
    ...
    private void promoteProductApi(String cardId, String packageId) {
      ...
      //  Log.e("PaymentDialog", "promoteProductApi: " + packageId + " : " + cardId + " : " + " : " + productId);
        WebService webService = Global.WebServiceConstants.getRetrofitinstance();
        Call<Common> stripePaymentApi = webService.promoteProductApi(auth, token, HelperPreferences.get(ShowCreditCardDetailFragment.this).getString(UID), productId, packageId, cardId, customer_id, "");
...
//        Log.e("card: ", cardId);
        Call<JsonObject> getProfileCall = service.set_default_card(auth, token, HelperPreferences.get(context).getString(UID), cardId);
